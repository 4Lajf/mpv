...
local o = {
	...
	-map 0:v -map 0:s:0 -map 0:a:m:language:jpn?
	-c:v libx264 -c:a mp3 -c:s mov_text
	-disposition:s:0 default -y 
	"$out.mp4"
    ]],
}
...
function get_outname(shift, endpos)
    ...
    name = name .. string.format("_%s-%s", timestamp(shift), timestamp(endpos))
	name = name:gsub(" ", "_")
    name = name:gsub(":", "-")
    return name
end
